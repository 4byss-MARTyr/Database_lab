...
SELECT COUNT() AS COUNT_, 
    ...
    MIN(price) AS MIN_, 
    MAX(price) AS MAX_, 
    YEAR(datee) AS DATE_ FROM `book` GROUP BY YEAR(datee);
...
SELECT COUNT() AS COUNT_, 
    ...
    MIN(price) AS MIN_, 
    MAX(price) AS MAX_, 
    YEAR(datee) AS DATE_ FROM `book` WHERE (price=10) and (price=20) GROUP BY YEAR(datee);
...
SELECT COUNT() AS COUNT_, 
    ...
    MIN(price) AS MIN_, 
    MAX(price) AS MAX_, 
    YEAR(datee) AS DATE_ FROM `book` GROUP BY YEAR(datee) ORDER BY COUNT_ DESC;

7. Вивести загальну кількість кодів книг і кодів книг що не повторюються
SELECT COUNT(),
    COUNT(cod),
    COUNT(DISTINCT cod) FROM `book`;
...
SELECT COUNT(),
    COUNT(price),
    left(name, 1) AS FIRSTLETTER FROM `book` WHERE NOT name LIKE '[A-Z]' AND NOT name LIKE '[0-9]'
    GROUP BY FIRSTLETTER;
...
SELECT COUNT() AS COUNT_, 
	...
	MAX(price) AS MAX_, 
	MONTH(datee) AS MONTH_, 
	YEAR(datee) AS YEAR_ FROM `books`
...
SELECT COUNT(),
    SUM(price) AS SUM_, 
    ROUND (price29.51, 0) AS UAH,
    ROUND (price0.91, 0) AS EUR, 
    ROUND (price105.10, 0) as RUB,
    new AS NEW FROM `book` GROUP BY NEW
...
SELECT COUNT() AS COUNT_, 
    ...
    MIN(price) AS MIN_, 
    MAX(price) AS MAX_,
    publisher AS PUBLISHER FROM `book` GROUP BY PUBLISHER;
...
SELECT COUNT() AS COUNT_, 
    ...
    MIN(price) AS MIN_, 
    MAX(price) AS MAX_, 
    topic, publisher FROM `book` GROUP BY topic, publisher 
    ORDER BY publisher;	
...
SELECT COUNT() AS COUNT_, 
    ...
    MIN(price) AS MIN_, 
    MAX(price) AS MAX_, 
    category, topic, publisher FROM `book` GROUP BY category, topic, publisher 
    ORDER BY category, topic, publisher;

20. Вивести список видавництв, у яких округлена до цілого ціна однієї сторінки більше 10 копійок.
SELECT publisher FROM `book` WHERE ROUND(price/pages)0.10	